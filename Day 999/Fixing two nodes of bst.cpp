...
class Solution {
public:
    void inorder(Node* root, Node *&p, Node* &first, Node* &sec){
        if(!root){
            return;
        }
        inorder(root->left, p, first, sec);
        ...
        p = root;
        inorder(root->right, p, first, sec);
    } 
  
    void correctBST( Node* root )
    {
        // add code here.
        Node *p = NULL, *first = NULL, *sec = NULL;
        inorder(root, p, first, sec);
